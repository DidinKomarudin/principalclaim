...
class PurchaseRecap(models.Model):
    _name = 'bsp.purchase.recap'
    _description = 'Monthly recap of purchase by branch and principal'
    _sql_constraints = [
        ('value_recap_uniq', 'unique (yearperiod,monthperiod,branch_code,principal_code)', 'This attribute value already exists !')
    ]
    ...
    partner_id = fields.Many2one('res.partner', 'Principal Name',
                                 required=False)
    hna_amount = fields.Float("HNA Amount")
    discount_amount = fields.Float("Discount Amount")
    ppn_amount = fields.Float("PPN Amount")
    total_amount = fields.Float("Net Amount")
    ...
    def create(self, vals):
        ...
        result = super(PurchaseRecap, self).create(vals)
        return result
    ...
    def write(self, vals):
        ...
        result = super(PurchaseRecap, self).write(vals)
        return result
